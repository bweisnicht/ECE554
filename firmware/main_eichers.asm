...
#        addi $2, $2,   1          # tmp = 1025
        lbi  $3,       0x002      #
        slbi $3,       0xffe      # counteraddr
        ld   $4, $3,   0          # counterdata <= [counteraddr]
        addi $4, $4,   0x1ff      # decrement counterdata
        nop                       #
        nop                       #
        st   $4, $3,   0          #
        bnez $4, .Jxfilter        # average samples
        st   $2, $3,   0          # reset counter
...
.Jfilterdo:                       #//START OF DO WHILE LOOP
 ...
 #>>>>>>>>>>>>>>>>>>>>>>>       ld   $5,  $2,  0          # filterdata <= [filteraddr]
        lbi  $7,       0x800      #
        slbi $7,       0x000      # temporary = 0x800000
        and  $11,  $5,  $7        # filterdata = filterdata & 0x800000
                                  #telling sign
        roli $11, $11, 5          #rotate it to the left 5 so we have 16
        sra  $8, $8, $11          #readjust blanks if negative
        sra  $9, $9, $11          #readjust bar if negative
        srai $11, $11, 4          # make the sign variable be only the lsb
        sub  $3, $11, $3          # decrement vga addr if we're negative
        beqz $11, .Jfilterpos     # jump if we're positive
        subi $5, $5, 0            # absolute value if negative
.Jfilterpos:        
        lbi  $7,       0x7ff      #
        slbi $7,       0xfff      # temporary = 0x7ffffff
        and  $5,  $5,  $7         # filterdata = filterdata & 0x7fffff
        addi $2,  $2,  1          # increment filteraddr
.Jfilterwhile1:                   #//START OF WHILE1 LOOOP STATEMENT
        beqz $5,  .Jfilterwhile2  # if(filterdata == 0); branch to while2

        subi  $12, $5, 0x001      # set equal for branch
        beqz $12, .Jfilterhalfbar # if this is equal, then we will override
        j .Jfilterfullbar         # if nothing else then its full
.Jfilterhalfbar:
        bnez $11, .Jfilterhalfneg
        lbi $7, 0x063             # load with half bar 
        slli $7, $7, 16           # shift over
        j .Jfilterif1
.Jfilterhalfneg:
        lbi $7, 0x06B             # load with half bar back
        j .Jfilterif1
.Jfilterfullbar:
        addi $7, $9, 0            # Load with full bar
.Jfilterif1:                      #//IF1 STATEMENT
        addi $12,  $4,  0x1fd      # tmp = 3count - 3
        bnez $12,  .Jfilterelse1   # if(3count != 3); branch to else1
                                  #//INSIDE IF1 STATEMENT
        addi $6,  $7,  0          # vgaout = appropriate bar
        addi $4,  $4,  0x1ff      # decrement 3count
        j    .Jfilterif2          # branch to if2       

.Jfilterelse1:                    #//ELSE1 STATEMENT
        or   $6,  $6,  $7         # vgaout = vgaout | appropriate bar
        addi $4,  $4,  0x1ff      # decrement 3count
.Jfilterif2:                      #//IF2 STATEMENT
        beqz $4,  .Jfilterelse2   # if(3count == 0); branch to else2
        bnez $11, .Jfilterneg1    # if we have a negative value
        srai $6,  $6,  8          # vgaout = vgaout >> 8
        j    .Jfilterendwhile1    # branch to end while1
.Jfilterneg1:
        slli $6, $6,   8          # vgaout = vgaout << 8
        j    .Jfilterendwhile1    # branch to end while1
.Jfilterelse2:                    #//ELSE2 STATEMENT
        ...
        j .Jfilterendwhile1
.Jfilterneg2:
        addi $3, $3, 0x1ff        #decrement vgaaddr
.Jfilterendwhile1:                #//UPDATE BEFORE END OF WHILE1
        addi $10, $10, 0x1ff      # decrement 12count
        srai $5,  $5,  2          # filterdata = filterdata >> 2
        j    .Jfilterwhile1       # jump to while1 statement    
.Jfilterwhile2:                   #//START OF WHILE2 LOOP STATEMENT
        beqz $10, .Jfilterprewhile # if(12count == 0); branch to prewhile
        addi $10, $10, 0x1ff      # decrement 12count
...
.Jfilterif4:                      #//IF4 STATEMENT
        ...
        bnez $11,  .Jfilterneg4    # we will shift left
        srai $6,  $6,  8          # vgaout = vgaout >> 8
        j    .Jfilterwhile2
.Jfilterneg4:
        slli $6, $6, 8            # vgaout = vgaout << 8
...
.Jfilterelse4:                    #//ELS42 STATEMENT
        st   $6,  $3,  0          # [vgaaddr] <= vgaout
        lbi  $4,       3          # 3count = 3
        bnez $11, .Jfilterneg5    # so we can decrement vgaaddr
        addi $3,  $3,  1          # increment vgaaddr
        j    .Jfilterwhile2
.Jfilterneg5:
        addi $3, $3, 0x1ff        # decrement vgaaddr
...
.Jfilterprewhile:                 #//PREWHILE STATEMENT
        addi $12, $8, 0           #fill a register with a blank
        rori $12, $12, 8          # rotate right
        or   $12, $8, $12         # or the result
        roli $7, $8, 8          # rotate left
        or   $7, $7, $12         # or the result
        bnez $11, .Jfilternegpre
        addi $3, $3, 0x1f8        #decrement 8 back to the start of the screen
        lbi  $12, 8               # value we need to get to the correct pos
        j .Jfilterprewhile2
.Jfilternegpre:
        addi $3, $3, 5           # increment back to middle of page
        lbi $12, 4               #correct offset to add back after storing
        slli  $9, $9, 16
        slli  $8, $8, 16
.Jfilterprewhile2:
        st $7, $3, 0              # store blanks
        st $7, $3, 1
        st $7, $3, 2
        st $7, $3, 3
        add $3, $3, $12           # increment vgaaddr so we are at the right
                                  # location 
        ...
        addi $3,  $3,  1          # increment vgaaddr
        st   $29, $3,  0          #
        subi $7,  $1,  8          #subtract by 8
        addi $7,  $7,  14         # increment it so we can have #s
        st   $7,  $3,  1          #
        addi $3,  $3,  0x1fa      # vgaaddr = vgaaddr - 7
        addi $3,  $3,  77         # vgaaddr  = vgaaddr + 77
...
.Jxfilterdo:                       #//START OF DO WHILE LOOP
        ...
        lbi  $7,       0x7ff      #
        slbi $7,       0xfff      # temporary = 0x7ffffff
        and  $25,  $25,  $7         # filterdata = filterdata & 0x7fffff




        srai $7,  $7,  10         # tmp = tmp >> 10#>>>>>>>>>>>>>>>>>>>>>>>
        addi $7,  $7,  1          # tmp = tmp + 1
        sub  $7,  $7,  $25         # tmp = filterdata - tmp
        bltz $7,  .Jsmallband     #
        #IMPORTANT INSTRUCTION THAT WAS REMOVED   srai $25,  $25,  10         # filterdata = filterdata >> 10#>>>>>>>>>>>>>>>>>>>>>>>
        #IMPORTANT INSTRUCTION THAT WAS REMOVED           add  $26, $26, $25         # bigdata = bigdata + filterdata
        srai $30, $30, 10        #IMPORTANT INSTRUCTION THAT REPLACED
        add $26, $26, $25         #IMPORTANT INSTRUCTION THAT REPLACED
        st   $26, $28,  0          # [bigaddr] <= bigdata
        j    .Jincrementsx        #
.Jsmallband:                      # |val| <= (2^23 - 1)
        #IMPORTANT INSTRUCTION THAT WAS REMOVED           add  $24, $24, $25         # smalldata = smalldata + filterdata
        add  $24, $24, $30         #IMPORTANT INSTRUCTION THAT REPLACED
...
#   filteraddr++
#   while(filterdata != 0){//while 1: display while
#     if(3count == 3){//if1
#       vgaout = 8bar
#       3count--
#     }
...
.Jtimer:                          #//START OF TIMER INTERRUPT
        ...
        j    .Jinterruptend       # return from timer interrupt
################################################################################
...
.Jflash:                          #//START OF COMPACT FLASH INTERRUPT
        ...
        lbi  $22,       0x005      #
        slbi $22,       0x008      # flashaddr = 0x005008
        lbi  $23,       0x006      #
